...
class AgentSkillSyncTests(unittest.TestCase):
    ...
    def test_stale_symlink_removed_when_skill_deleted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.make_skill(source, "atlas")
            sync.sync_symlink_target(root, source, ".claude/skills", ["atlas"])
            # Skill removed from source; re-sync with an empty skill list
    ...
    def test_real_file_never_touched(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (dest / "atlas").mkdir()  # a real directory, not our symlink
    ...
    def test_agy_manifest_points_at_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = root / ".github" / "skills"
            self.make_skill(source, "atlas")
            ...
            self.assertEqual(manifest, {"entries": [{"path": ".github/skills"}]})
